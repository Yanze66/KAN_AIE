...
void xavier_initialize(Univariate *uni, int size) {
    double xavier = sqrt(2.0 / size);
    uni->c = (rand() / (double)RAND_MAX) * 2.0 * xavier - xavier;
...
double Univariate_GetFunctionUsingInput(Univariate *uni, double x) {
    ...
    // printf("x: %f, leftIndex: %d, leftOffset: %f\n", x, leftIndex, leftOffset);
    return uni->y[leftIndex] + (uni->y[leftIndex + 1] - uni->y[leftIndex]) * leftOffset + uni->c * relu(x);
}
